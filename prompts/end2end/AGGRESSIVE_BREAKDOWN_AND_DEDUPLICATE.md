# TASK (END-TO-END): Identify and refactor ALL functions exceeding complexity thresholds AND consolidate ALL code clone groups — no cap per session.
...
**End-to-End Stopping Conditions** (override any earlier stopping rules):
- ✅ **All** flagged items resolved — every item above threshold is below threshold (or justified as an exception).
- ⚠️ Context boundary reached — commit progress, document all remaining items explicitly, resume in next session.
- 🛑 Unrecoverable regression — revert the last change, document the blocker, stop.
Do NOT stop because a fixed count has been reached. Do NOT sample only the top N.
### Phase 4: Validate
```bash
...
- Each extracted helper for complexity must be <20 lines, cyclomatic <8.
