# TASK (END-TO-END): Perform a universal, comprehensive bug-hunting audit of a Go project — no domain is off-limits, no finding cap.

## Execution Mode
**Report generation only** — do NOT modify any source code.

## End-to-End Policy
This is the **end-to-end variant** of GENERIC_AUDIT. The following rules are absolute:
- **No finding cap** — report every confirmed finding regardless of total count.
...
### Phase 3: Systematic Bug Hunt — Full Coverage Pass

**End-to-end rule**: Inspect every code site for each category below. Do not skip files or packages. Do not stop when a finding count looks high. Process everything.
...
- Packages with doc coverage <50% may have undocumented behavioral contracts — inspect ALL exported APIs in those packages.
...
#### 3l. False-Positive Prevention (MANDATORY — applies even in end-to-end mode)
...
**End-to-end note**: The false-positive filter does NOT reduce coverage — it improves quality. Apply it diligently. A thorough, accurate audit with 200 findings is better than a sloppy one with 20.

### Phase 4: Report

Generate **`AUDIT.md`**:
```markdown
# UNIVERSAL BUG AUDIT (END-TO-END) — [date]
...
## Coverage Log
[List each package audited; mark each checklist category completed per package]
...
## Constraints
- Do not cap findings by count — report **every** finding that meets the threshold, regardless of total count.
...
## Session Strategy
- Audit one package at a time; complete all checklist categories for a package before moving on.
- Update the Coverage Log in `AUDIT.md` after each package to track progress.
