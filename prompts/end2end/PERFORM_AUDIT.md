# TASK (END-TO-END): Perform a comprehensive goal-focused functional audit of the Go codebase using `go-stats-generator` metrics as the primary evidence source.
...
5. Cross-reference with `.duplication.clones` (clone groups with `line_count` and `instances`) and `.documentation` for additional findings.
