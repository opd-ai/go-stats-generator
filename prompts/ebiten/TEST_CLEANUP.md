# TASK: [Ebitengine Edition] Remove redundant test artifacts, consolidate duplicate test helpers, and clean up test infrastructure.
...
- **Pure Game Logic**: Separate game logic from rendering for unit testing
- **Interface Abstractions**: `Image`, `Input`, and `Audio` as interfaces for mocking
- **Deterministic Simulation**: Fixed time steps for reproducible tests
- **Headless Testing**: Tests should not require windowing system

### Common Testing Challenges
- Cannot instantiate `ebiten.Image` outside game loop (use interfaces)
- Input state requires frame-by-frame simulation
- Physics determinism requires fixed delta time
- Audio testing requires mock players
...
- Use `.duplication.clones[]` to find test files with >30 duplicated lines.
