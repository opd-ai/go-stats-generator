# TASK: [Ebitengine Edition] Perform a focused audit of Ebitengine game code for initialization order and use-before-ready logic errors in Go code, identifying use-before-initialization, incorrect operation ordering, nil map/slice write panics, zero-value assumption violations, circular initialization, and time-of-check to time-of-use (TOCTOU) logic bugs while rigorously preventing false positives.
