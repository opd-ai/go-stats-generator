...
```
**Section filter**: Use only `.functions` from the report. Exclude `.structs`, `.interfaces`, `.packages`, `.concurrency`, `.duplication`, `.naming`, `.documentation`, `.placement`, `.organization`, `.burden`, `.scores`, `.generics`, `.patterns`, `.suggestions` — they are not relevant to function complexity refactoring.
